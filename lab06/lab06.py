...
class Transaction:
    ...
    def report(self) -> str:
        ...
        if self.changed():
            msg = f"decreased {self.before}->{self.after}" if self.before > self.after else f"increased {self.before}->{self.after}"
...
class Coin:
    ...
    def worth(self) -> int:
        age = abs(self.year-Mint.present_year)
...
class VirFib():
    ...
    def next(self):
        ...
        if self.value == 0:
            ...
            new.value = self.value + self.prev
            new.prev = self.value
        return new
