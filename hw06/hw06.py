...
def store_digits(n: int):
    ...
    # Helper to find highest power of 10 less than or equal to n
    power = 1
    while floordiv(n, power) >= 10:
        power *= 10
    ...
    def helper(num, p):
        if p == 0:
            return Link.empty
        first_digit = floordiv(num, p)
        rest = mod(num, p)
        return Link(first_digit, helper(rest, p // 10))
...
def prune_small(t, n):
    ...
    for b in t.branches:
        prune_small(b, n) if not Tree.is_leaf(b) else 1+2
